...
public class ExtendedWaterTimeHudElement implements HudElement {
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, DeltaTracker deltaTracker) {
		...
		if (player != null && !player.isSpectator()) {
			ExtendedWaterTimeComponent extendedWaterTime = EnchancementEntityComponents.EXTENDED_WATER_TIME.get(player.getControlledVehicle() instanceof LivingEntity living ? living : player);
			int ticksWet = extendedWaterTime.getTicksWet();
			if (ticksWet > 0) {
				int x = graphics.guiWidth() / 2 - 11, y = graphics.guiHeight() / 2 + 5;
				float progress = Math.min(1, Mth.lerp((extendedWaterTime.getTicksWet() + 1) / (float) extendedWaterTime.getLastMarked(), 0, 1));
				if (progress > 0) {
