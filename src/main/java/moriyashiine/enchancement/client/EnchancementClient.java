...
public class EnchancementClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		FabricModelPredicateProviderRegistry.register(Items.CROSSBOW, new Identifier(Enchancement.MOD_ID, "crossbow_brimstone"), (stack, world, entity, seed) -> CrossbowItemAccessor.enchancement$getProjectiles(stack).stream().anyMatch(foundStack -> ItemStack.areEqual(foundStack, EnchancementUtil.BRIMSTONE_STACK)) ? 1 : 0);
		FabricModelPredicateProviderRegistry.register(Items.CROSSBOW, new Identifier(Enchancement.MOD_ID, "crossbow_torch"), (stack, world, entity, seed) -> CrossbowItem.hasProjectile(stack, Items.TORCH) ? 1 : 0);
		ScreenRegistry.register(ModScreenHandlerTypes.ENCHANTING_TABLE, EnchantingTableScreen::new);
