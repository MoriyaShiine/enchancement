...
public record AddMoltenParticlesPayload(BlockPos pos) implements CustomPayload {
	...
	public static class Receiver implements ClientPlayNetworking.PlayPayloadHandler<AddMoltenParticlesPayload> {
		...
		public void receive(AddMoltenParticlesPayload payload, ClientPlayNetworking.Context context) {
			...
			if (world != null) {
				for (int i = 0; i < 8; i++) {
					world.addParticleClient(ModParticleTypes.SHORT_SMALL_FLAME, payload.pos().getX() + 0.5 + MathHelper.nextDouble(world.random, -0.5, 0.5F), payload.pos().getY() + 0.5 + MathHelper.nextDouble(world.random, -0.5, 0.5F), payload.pos().getZ() + 0.5 + MathHelper.nextDouble(world.random, -0.5, 0.5F), 0, 0, 0);
				}
			}
		}
	}
