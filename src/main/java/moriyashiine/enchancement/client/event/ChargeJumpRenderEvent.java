		...
		ModEntityComponents.CHARGE_JUMP.maybeGet(MinecraftClient.getInstance().getCameraEntity()).ifPresent(chargeJumpComponent -> {
			if (chargeJumpComponent.hasChargeJump()) {
				...
				if (boostProgress > 0) {
					...
					drawContext.drawGuiTexture(RenderLayer::getGuiTextured, BACKGROUND_TEXTURE, x, y, 182, 5);
					drawContext.drawGuiTexture(RenderLayer::getGuiTextured, PROGRESS_TEXTURE, 182, 5, 0, 0, x, y, (int) (182 * boostProgress), 5);
				}
			}
		});
	}
