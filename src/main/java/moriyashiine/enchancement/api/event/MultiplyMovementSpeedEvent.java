...
public interface MultiplyMovementSpeedEvent {
	...
	static float getMovementMultiplier(LivingEntity entity) {
		...
		if (entity.isUsingItem() && !entity.isPassenger()) {
			multiplier *= EnchancementUtil.getItemUseSpeedMultiplier(entity.getUseItem(), entity.getUseItem().getOrDefault(DataComponents.USE_EFFECTS, UseEffects.DEFAULT).speedMultiplier());
		}
		if (entity.isSprinting()) {
			multiplier /= 1.3;
		}
