...
public class FellTreesComponent implements ServerTickingComponent {
	...
	public FellTreesComponent(Level obj) {
		this.obj = obj;
	}
	...
	public void readData(ValueInput input) {
		treesToCut.clear();
		for (Tree tree : input.read("TreesToCut", Tree.CODEC.listOf()).orElse(List.of())) {
	...
	public void serverTick() {
		for (int i = treesToCut.size() - 1; i >= 0; i--) {
			...
			if (tree.logs.isEmpty()) {
				EnchancementUtil.mergeItemEntities(tree.drops.stream().map(drop -> new ItemEntity(obj, tree.originalPos.getX() + 0.5, tree.originalPos.getY() + 0.5, tree.originalPos.getZ() + 0.5, drop)).collect(Collectors.toList())).forEach(obj::addFreshEntity);
				treesToCut.remove(i);
				continue;
			}
			BlockPos pos = tree.logs.removeLast();
			tree.drops.addAll(Block.getDrops(obj.getBlockState(pos), (ServerLevel) obj, pos, obj.getBlockEntity(pos), null, tree.stack));
