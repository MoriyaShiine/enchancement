...
public record BoostInFluidPayload(int entityId, boolean shouldBoost) implements CustomPayload {
	public static final CustomPayload.Id<BoostInFluidPayload> ID = new Id<>(Enchancement.id("boost_in_fluid"));
	public static final PacketCodec<PacketByteBuf, BoostInFluidPayload> CODEC = PacketCodec.tuple(PacketCodecs.VAR_INT, BoostInFluidPayload::entityId, PacketCodecs.BOOLEAN, BoostInFluidPayload::shouldBoost, BoostInFluidPayload::new);
	...
	public static void send(Entity entity, boolean shouldBoost) {
		ClientPlayNetworking.send(new BoostInFluidPayload(entity.getId(), shouldBoost));
	}
