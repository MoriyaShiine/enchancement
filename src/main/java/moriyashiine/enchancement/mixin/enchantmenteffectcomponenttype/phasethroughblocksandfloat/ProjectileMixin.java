...
public class ProjectileMixin {
	...
	private static <T extends Projectile> Projectile enchancement$phaseThroughBlocksAndFloat(T original) {
		if (original.getOwner() instanceof LivingEntity owner) {
			PhaseThroughBlocksAndFloatComponent.maybeSet(owner, owner.getActiveItem(), original);
		}
		return original;
	}
}
